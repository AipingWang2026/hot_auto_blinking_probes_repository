...
% [mask.BW,mask.Area] = Locs2Mask( xy(:,1:2));
[mask.BW,mask.Area] = Locs2Mask( xy(:,1:2), finalpix, pix2nm );
%     figure, imshow(mask.BW), title('Mask based on original localizations'),set(gca,'XLim',[1245 1487],'YLim',[350 590])
%     figure, imshow(mask.BW)
%     hold on
%     % plot voronoi diagram
%     hv = voronoi(xyorig(:,1)*pix2nm/finalpix,xyorig(:,2)*pix2nm/finalpix,'m');
%     % change color of voronoi diag
%     for i=1:2
%         set(hv(i),'Color',[0.9290    0.6940    0.1250])%[0 0.6 0])
%     end
%     title('Original mask with voronoi plot of all original localizations')
%     set(gca,'XLim',[1245 1487],'YLim',[350 590])
% density = npts/mask.Area
% %%
...
% Nbins = 2*round(2*npts^(1/3));%round(sqrt(npts));%
% if ~exist('areaLim','var') || isempty(areaLim)
%     areaLim = 2*4*median(Vareas);
% end
Vareas = Vareas(Vareas<areaLim);
% % nptsAnalyzed = size(Vareas,1);
% [counts, centers] = hist(Vareas,Nbins);
% [counts, centers] = histcounts(Vareas, 'BinMethod','fd');
...
%     figure, imshow(maskDil), title('Mask dilated from original mask'),set(gca,'XLim',[1245 1487],'YLim',[350 590])
...
counts_rnd = nan(iter,length(counts));%nan(iter,Nbins);
...
for j = 1:iter
    ...
    % toc
    %         figure, imshow(maskSq), title('Mask based on randomized localizations'),set(gca,'XLim',[1245 1487],'YLim',[350 590])
    
    %% select random locs inside the dilated mask
    %         PtsRndRd = round(ptsRnd(:,1:2)*(pix2nm/finalpix));
    % tic
    kp = pixLocs( maskDil );
    kp = cell2mat(kp);
    kp = sort(kp);
    
    %         disp('End selection for dilated mask')
    
    %         % plot mask of localizations within dilated mask
    %         maskRnd = Locs2Mask_beta(ptsRnd(kp,1:2));
    %         figure, imshow(maskRnd), title('Mask Based on randomized localizations within dilated mask'),set(gca,'XLim',[1245 1487],'YLim',[350 590])
    ...
    ptsRnd = ptsRnd(kp,:);
    %         disp('End selection for original mask')
    % toc
    %         maskRnd = Locs2Mask(ptsRnd(:,1:2));%*(pix2nm/finalpix));
    %         figure, imshow(maskRnd), title('Mask Based on randomized localizations within ORIGINAL mask'),set(gca,'XLim',[1245 1487],'YLim',[350 590])
    
    %% determine histogram for random localizations
    Varea_rnd{j} = ptsRnd(~isnan(ptsRnd(:,3)),3);
    %         Varea_rnd{j} = Varea_rnd{j}(Varea_rnd{j}<areaLim);
    %         [counts_rnd(j,:), centers_rnd(j,:)] = hist(Varea_rnd{j},Nbins);
...
%                   centers_rnd', MeanCounts', ...
%                   (MeanCounts - z * StdCounts)', (MeanCounts + z * StdCounts)'];
